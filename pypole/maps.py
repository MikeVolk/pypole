...
def get_random_sources(
    ...
    locations = get_random_locations(n_maps, n_sources, x_range, y_range, z_range)
    # reshape total location to be (n_maps, n_sources, (x,y,z))
    locations = locations.reshape(n_maps, n_sources, 3)

    # calculate the x/y/z components
    dim = get_random_dim(moment_range, n_maps, n_sources)
    xyz = convert.dim2xyz(dim)

    # reshape xyz to be (n_maps, n_sources, (x,y,z))
    source_vector = xyz.reshape(n_maps, n_sources, 3)

    return locations, source_vector


def get_random_dim(moment_range, n_maps, n_sources):
    ...
    declination = np.random.uniform(0, 360, n_maps * n_sources)
    # get random inclinations from uniform distribution (rand)
    inclination = np.rad2deg(np.arccos(2 * np.random.rand(n_maps * n_sources) - 1)) - 90
    # get uniform distribution of moment magnitude
    moment_scalar = np.random.uniform(
        moment_range[0], moment_range[1], size=n_maps * n_sources
    )
    return np.stack([declination, inclination, moment_scalar]).T


def get_random_locations(n_maps, n_sources, x_range, y_range, z_range):
    ...
    x_source = np.random.uniform(x_range[0], x_range[1], size=(n_maps * n_sources))
    y_source = np.random.uniform(y_range[0], y_range[1], size=(n_maps * n_sources))
    z_source = np.random.uniform(z_range[0], z_range[1], size=(n_maps * n_sources))
    return np.stack([x_source, y_source, z_source]).T


def get_grid(n_pixel: int = 100, pixel_size: float = 5e-6):
